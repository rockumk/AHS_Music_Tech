...
process_the_form = function(user_header_entry, user_progression_entry)
	...
	vr = ""
	errormessage = ""
	G_original_user_entry = user_progression_entry
	G_form_table = nil
	G_section_table = {}
	-- __________________________________________  Find The Form and enter Each Section into a Table
	function form_to_table(header_entry)
		header_entry = header_entry .. string.char(10)
		form_table = {}
		_, form_endchar = string.find(header_entry, "Form:")
		if form_endchar == nil then
					--reaper.ShowConsoleMsg("Dang!!!!!!!!!!!!! NIL!!!!!!!!!!!!!!!" .. string.char(10))
					--reaper.ShowConsoleMsg(header_entry .. string.char(10))	
					
		else
			return_char_location,_ = string.find((string.sub(header_entry, form_endchar,-1)), string.char(10))
			if return_char_location == nil then
			--reaper.ShowConsoleMsg("Dang!!!!!!!!!!!!! MISSING!!!!!!!!!!!!!!!" .. string.char(10))
			else
				the_form = string.char(10) .. string.sub(header_entry, form_endchar +1, form_endchar + return_char_location -2)
				quote_active = false
				the_quote = ""
				form_table = {}
	...
	function sections_to_table(user_progression_entry)
		section_table = {}
		all_sections_data = user_progression_entry
		if verbose == true then errormessage = errormessage .. "--------------" .. all_sections_data ..  "--------------" .. string.char(10) end
		brace_count = 0
		brace_char_location, _ = string.find(all_sections_data, "{")
