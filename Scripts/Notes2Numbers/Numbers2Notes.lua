...
local verylightgray = reaper.ImGui_ColorConvertDouble4ToU32(.9, .9, .9, 1)
real_song_key = "C"
processing_key = "C"
...
if rv1 > 0 then
    G_render_mode = tonumber(saved_mode)
end

local rv2, saved_drum_cue = reaper.GetProjExtState(0, "N2N", "DrumCueMode")
if rv2 > 0 and saved_drum_cue ~= "" then
    G_DRUM_CUE_PLACEMENT = saved_drum_cue
end

local rv3, saved_arp_cue = reaper.GetProjExtState(0, "N2N", "ArpCueMode")
if rv3 > 0 and saved_arp_cue ~= "" then
    G_ARP_CUE_PLACEMENT = saved_arp_cue
end

-- Inject loaded states into the UI Recipe so dropdowns match upon boot
for _, tr in ipairs(config.track_recipe) do
    if tr.type == 32 and rv2 > 0 then
        tr.drum_arp_mode = saved_drum_cue
    end
    if tr.type == 33 and rv3 > 0 then
        tr.drum_arp_mode = saved_arp_cue
    end
end
...
function Ensure_Audition_Track_Exists()
    ...
    local lex_idx = reaper.TrackFX_AddByName(new_track, "Red Lexikan S2", false, -1)
    if lex_idx >= 0 then
        reaper.TrackFX_SetPreset(new_track, lex_idx, "Plate Nicer Things")
    end
    ...
    if reaper.ImGui_BeginPopupModal(ctx, "Configure N2N Tracks", true) then
        ...
        if reaper.ImGui_BeginChild(ctx, "track_list_scroll", 0, 580, 0) then
            for i, tr in ipairs(track_recipe) do
                ...
                local field =
                    tr.vsti_list and "vsti_choice" or
                    (tr.vst_list and "vst_choice" or (tr.audio_list and "audio_choice" or nil))
                ...
                if tr.drum_arp_mode then
                    ...
                    if arp_mode_changed then
                        ...
                        if tr.type == 32 then
                            reaper.SetProjExtState(0, "N2N", "DrumCueMode", new_arp_mode)
                        end
                        if tr.type == 33 then
                            reaper.SetProjExtState(0, "N2N", "ArpCueMode", new_arp_mode)
                        end
                    end
                    dropdown_x_offset = dropdown_x_offset + 340
                end
...
----------------------------------------------- STYLE COLOR MANAGER
local n2n_styles = {
    {reaper.ImGui_Col_WindowBg(), 0x506070FF},
...
local function Draw_Sticky_Mini_Chord_Bar(context)
    ...
    -- === TWEAKABLE LAYOUT VARIABLES ==================================
    local helper_text_y_offset = 8 -- Nudges the "See Entry tab..." text
    local labels_y_offset = -11 -- Nudges the black F1-F6 labels (Negative = UP relative to the button)
    local buttons_y_offset = 2 -- Nudges the colored chord buttons

    local button_width = 25
    local button_height = 18
    local start_x_buttons = 504 -- Where the first button starts
    local horizontal_spacing = 31 -- Space between each button block
    -- =================================================================

    -- Anchor ourselves to the current line (the Tab Bar)
    reaper.ImGui_SameLine(context, 270)
    local anchor_y = reaper.ImGui_GetCursorPosY(context)

    -- Draw Helper Text
    reaper.ImGui_SetCursorPosY(context, anchor_y + helper_text_y_offset)
    reaper.ImGui_PushFont(context, fontsmall)
    reaper.ImGui_PushStyleColor(context, reaper.ImGui_Col_Text(), verylightgray)
    reaper.ImGui_Text(context, 'See "Entry" tab for less common chords')
    reaper.ImGui_PopFont(context)
    reaper.ImGui_PopStyleColor(context, 1)
    local quick_chords = {
        {label = "1", val = {"", "       ", {0, 4, 7}}, fkey = "F1", imkey = reaper.ImGui_Key_F1()},
        {label = "2m", val = {"m", "m      ", {0, 3, 7}}, fkey = "F2", imkey = reaper.ImGui_Key_F2()},
        {label = "3m", val = {"m", "m      ", {0, 3, 7}}, fkey = "F3", imkey = reaper.ImGui_Key_F3()},
        {label = "4", val = {"", "       ", {0, 4, 7}}, fkey = "F4", imkey = reaper.ImGui_Key_F4()},
        {label = "5", val = {"", "       ", {0, 4, 7}}, fkey = "F5", imkey = reaper.ImGui_Key_F5()},
        {label = "6m", val = {"m", "m      ", {0, 3, 7}}, fkey = "F6", imkey = reaper.ImGui_Key_F6()}
    }
    local quick_roots = {"1", "2", "3", "4", "5", "6"}

    -- Grab the DrawList to paint text outside the layout engine
    local draw_list = reaper.ImGui_GetWindowDrawList(context)
    local black_color = reaper.ImGui_ColorConvertDouble4ToU32(0, 0, 0, 1)

    for i, c in ipairs(quick_chords) do
        local current_root = quick_roots[i]
        local clean_name = c.val[1]
        local cur_x = start_x_buttons + ((i - 1) * horizontal_spacing)

        -- Position the cursor for the BUTTON
        reaper.ImGui_SetCursorPosY(context, anchor_y + buttons_y_offset)
        reaper.ImGui_SameLine(context, cur_x)

        -- Grab the absolute screen position where the button WILL be drawn
        local btn_screen_x, btn_screen_y = reaper.ImGui_GetCursorScreenPos(context)

        -- Draw the F-Key Label manually as a painted overlay! (Bypasses Layout limitations)
        reaper.ImGui_PushFont(context, fontsmall)
        local f_text_w = reaper.ImGui_CalcTextSize(context, c.fkey)
        reaper.ImGui_PopFont(context)

        local centered_x = btn_screen_x + ((button_width - f_text_w) / 2)
        local label_screen_y = btn_screen_y + labels_y_offset
        ...
        -- Keyboard States
        local f_pressed = reaper.ImGui_IsKeyPressed(context, c.imkey, false)
        local f_released = reaper.ImGui_IsKeyReleased(context, c.imkey)
        local is_held = reaper.ImGui_IsKeyDown(context, c.imkey)

        -- Color Logic (Lights up brightly if the F-Key is currently held down)
        local root_colors = musictheory.root_colors[current_root] or {200, 200, 200}
        local r_val, g_val, b_val = root_colors[1] / 255, root_colors[2] / 255, root_colors[3] / 255

        if is_held then
            r_val = math.min(r_val + 0.4, 1.0)
            g_val = math.min(g_val + 0.4, 1.0)
            b_val = math.min(b_val + 0.4, 1.0)
        end

        local thecolor = reaper.ImGui_ColorConvertDouble4ToU32(r_val, g_val, b_val, 1)
        reaper.ImGui_PushStyleColor(context, reaper.ImGui_Col_Button(), thecolor)

        -- Draw the Button
        reaper.ImGui_Button(context, c.label, button_width, button_height)
        reaper.ImGui_PopStyleColor(context, 1)

        -- Trigger logic on Mouse Click OR Keyboard Press
        if reaper.ImGui_IsItemActivated(context) or f_pressed then
            if is_ctrl_down then
                if charting_tab_mode == 1 then
                    chord_charting_area = chord_charting_area .. current_root .. clean_name .. "  "
                elseif charting_tab_mode == 7 then
                    local cur_key = set_the_key(header_area)
                    local fake_chord = current_root .. clean_name
                    local letter_chord = numbers_to_letters(cur_key, fake_chord)
                    chord_charting_area_letters = chord_charting_area_letters .. letter_chord .. "  "
                end
            end
        ...
        -- Release logic on Mouse Up OR Keyboard Release
        if reaper.ImGui_IsItemDeactivated(context) or f_released then
            Kill_All_Audition_Notes()
        end
    end

    -- Restore the cursor to the baseline so the Tab bar finishes rendering cleanly
    reaper.ImGui_SetCursorPosY(context, anchor_y)
end

function Action_SetupAndRender()
    if charting_tab_mode == 7 then
        local cur_key = set_the_key(header_area)
        chord_charting_area = letters_to_numbers(cur_key, chord_charting_area_letters)
    end
    is_track_builder_open = true
    track_builder_was_open = false -- FIX: Forces the window to trigger even if closed previously!
end
...
function Action_Update()
    ...
    for _, tr in ipairs(config.track_recipe) do
        if tr.active then has_active_tracks = true break end
    end

    if has_active_tracks then
        if not render_co then Start_Render_Coroutine() end
    else
        is_track_builder_open = true
        track_builder_was_open = false
    end
end

function Jump_To_Region_Index(target_idx, target_name, measure_offset)
    local target_pos = nil
    local _, num_markers, num_regions = reaper.CountProjectMarkers(0)
    local total = num_markers + num_regions
    
    -- 1. Parse name and instance (e.g. "C2" -> "C", 2)
    local base_name = target_name
    local target_instance = 1
    local parsed_name, parsed_num = target_name:match("^(%a+)%s*(%d*)$")
    if parsed_name then
        base_name = parsed_name
        if parsed_num and parsed_num ~= "" then target_instance = tonumber(parsed_num) end
    end
    
    -- Expand abbreviations like place_special does (e.g., "C" -> "Chorus")
    local expanded_name = base_name
    local u = base_name:upper()
    if u:match("^V") then expanded_name="Verse" elseif u:match("^C") then expanded_name="Chorus"
    elseif u:match("^P") then expanded_name="Pre-Chorus" elseif u:match("^B") then expanded_name="Bridge"
    elseif u:match("^O") then expanded_name="Outro" elseif u:match("^I") then expanded_name="Intro"
    elseif u:match("^M") then expanded_name="Middle 8" elseif u:match("^R") then expanded_name="Ramp"
    elseif u:match("^D") then expanded_name="Drop" elseif u:match("^S") then expanded_name="Solo"
    elseif u:match("^F") then expanded_name="Fadeout" end
    
    local exact_search_name = expanded_name
    if target_instance > 1 or target_name:match("%d+$") then exact_search_name = expanded_name .. " " .. target_instance end
    
    -- STRATEGY A: Exact Name Match (e.g., "Chorus 2")
    for i = 0, total - 1 do
        local retval, isrgn, pos, rgnend, name = reaper.EnumProjectMarkers3(0, i)
        if isrgn and name:lower() == exact_search_name:lower() then
            target_pos = pos; break
        end
    end
    
    -- STRATEGY B: Fallback to the Nth instance of the base name (e.g., 2nd "Chorus")
    if not target_pos then
        local instance_count = 0
        for i = 0, total - 1 do
            local retval, isrgn, pos, rgnend, name = reaper.EnumProjectMarkers3(0, i)
            if isrgn and name:lower():match("^" .. expanded_name:lower()) then
                instance_count = instance_count + 1
                if instance_count == target_instance then
                    target_pos = pos; break
                end
            end
        end
    ...
    -- STRATEGY C: Fallback to Chronological Index (original logic)
    if not target_pos then
        local current_region_idx = 0
        for i = 0, total - 1 do
            local retval, isrgn, pos, rgnend, name = reaper.EnumProjectMarkers3(0, i)
            if isrgn then
                current_region_idx = current_region_idx + 1
                if current_region_idx == target_idx then
                    target_pos = pos; break
                end
            end
        end
    ...
    if not target_pos then
        render_feedback = render_feedback .. "\n[" .. os.date("%H:%M:%S") .. "] Section '" .. target_name .. "' not found. Render first.\n"
        return
    end
    
    -- Exact Measure Math fix
    if measure_offset < 0 then
        local retval, measures, cml, fullbeats, cdenom = reaper.TimeMap2_timeToBeats(0, target_pos)
        local target_measure = measures + measure_offset
        if target_measure < 0 then target_measure = 0 end
        target_pos = reaper.TimeMap2_beatsToTime(0, 0, target_measure)
    end
    
    reaper.Main_OnCommand(1016, 0) -- Stop playback to kill hanging notes!
...
function Get_Transport_Sections(header_text, chord_text)
    local sections = {}
    local form_line = header_text:match("Form:%s*(.-)\n")
    if not form_line then form_line = header_text:match("Form:%s*(.*)$") end
    if not form_line then return sections end
    
    -- Compile a list of defined sections from the chord text to check for "Missing"
    local defined_sections = {}
    if chord_text then
        for sec in chord_text:gmatch("{([^}]+)}") do
            defined_sections[sec:match("^%s*(.-)%s*$")] = true
        end
    end
    ...
    while s:len() > 0 do
        s = s:match("^%s*(.*)")
        if s:len() == 0 then break end
        if s:sub(1,1) == "{" then
            local e = s:find("}")
            if e then table.insert(tokens, s:sub(2, e-1)); s = s:sub(e+1)
            else table.insert(tokens, s:sub(2)); break end
        elseif s:sub(1,2) == "{$" then
            local e = s:find("%$}")
            if e then table.insert(tokens, s:sub(3, e-1)); s = s:sub(e+2)
            else table.insert(tokens, s:sub(3)); break end
        else
            local e = s:find("%s")
            if e then table.insert(tokens, s:sub(1, e-1)); s = s:sub(e)
            else table.insert(tokens, s); break end
        end
    end

    local rgn_idx = 1
    for _, token in ipairs(tokens) do
        local upper_token = token:upper()
        local base_name = token
        
        if upper_token:match("^V%d*$") or upper_token == "VERSE" then base_name = "Verse"
        elseif upper_token:match("^C%d*$") or upper_token == "CHORUS" then base_name = "Chorus"
        elseif upper_token:match("^P%d*$") or upper_token:match("^PC%d*$") or upper_token:match("^PRE[- ]?") then base_name = "Pre-Chorus"
        elseif upper_token:match("^B%d*$") or upper_token == "BRIDGE" then base_name = "Bridge"
        elseif upper_token:match("^O%d*$") or upper_token == "OUTRO" then base_name = "Outro"
        elseif upper_token:match("^I%d*$") or upper_token:match("^IN%d*$") or upper_token == "INTRO" then base_name = "Intro"
        elseif upper_token:match("^M%d*$") or upper_token == "MIDDLE 8" then base_name = "Middle 8"
        elseif upper_token:match("^R%d*$") or upper_token == "RAMP" then base_name = "Ramp"
        elseif upper_token:match("^D%d*$") or upper_token == "DROP" then base_name = "Drop"
        elseif upper_token:match("^S%d*$") or upper_token == "SOLO" then base_name = "Solo"
        elseif upper_token:match("^F%d*$") or upper_token == "FADEOUT" then base_name = "Fadeout"
        elseif upper_token == "#" then base_name = "#"
        end
        
        local is_missing = false
        if not defined_sections[token] then
            local fallback_base = token:match("^(%a+)%s*%d*$")
            if not (fallback_base and defined_sections[fallback_base]) then
                is_missing = true
            end
        end

        local r, g, b = 80, 80, 100
        if form and form.sections_colors then
            local c_vals = form.sections_colors[base_name] or form.sections_colors[token]
            if c_vals then r, g, b = c_vals[1], c_vals[2], c_vals[3] end
        end
        ...
        if not is_missing then rgn_idx = rgn_idx + 1 end
    end
    return sections
end 
...
function IM_GUI_Loop()
    ...
    -- Execute Delayed Audition Mute
    if audition_mute_time and reaper.time_precise() > audition_mute_time then
        if audition_track and reaper.ValidatePtr(audition_track, "MediaTrack*") then
            reaper.SetMediaTrackInfo_Value(audition_track, "B_MUTE", 1)
        end
        audition_mute_time = nil
    end
    
        -- Handle global Hotkeys
        local is_shift_down = reaper.ImGui_IsKeyDown(ctx, reaper.ImGui_Mod_Shift())

        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_F7(), false) then
            Action_SetupAndRender()
        end
        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_F8(), false) then
            Action_Update()
        end

        -- Transport Hotkeys
        if is_shift_down and reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_1(), false) then
            reaper.Main_OnCommand(40349, 0)
        end -- Panic (!)
        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_F9(), false) then
            reaper.Main_OnCommand(1013, 0)
        end -- Record
        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_F10(), false) then
            reaper.Main_OnCommand(1007, 0)
        end -- Play
        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_F11(), false) then
            reaper.Main_OnCommand(1016, 0)
        end -- Stop
        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_F12(), false) then
            reaper.Main_OnCommand(1008, 0)
        end -- Pause
        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_PageDown(), false) then
            reaper.Main_OnCommand(40172, 0)
        end -- Prev
        if reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_PageUp(), false) then
                ...
                if G_startup_missing_report ~= "" then
                    ...
                    if render_co then
                        if coroutine.status(render_co) ~= "dead" then
                            ...
                            render_co = nil
                            modal_on = false
                            reaper.Undo_EndBlock("Numbers2Notes: Render Chart", -1) 
                            r.ImGui_CloseCurrentPopup(ctx)
                        end
                    end
                end
        ...
        reaper.ImGui_Dummy(ctx, 0, 2) -- Change '8' to make the gap bigger or smaller
        -- Start the tabs
        r.ImGui_BeginTabBar(ctx, "Charting", r.ImGui_TabBarFlags_None())

        if r.ImGui_BeginTabItem(ctx, "Chord #") then
            if last_charting_tab == 7 then
                local cur_key = set_the_key(header_area)
                chord_charting_area = letters_to_numbers(cur_key, chord_charting_area_letters)
            end
            last_charting_tab = 1
        ...
        if r.ImGui_BeginTabItem(ctx, "Letters") then
            if last_charting_tab == 1 then
                local cur_key = set_the_key(header_area)
                chord_charting_area_letters = numbers_to_letters(cur_key, chord_charting_area)
            end
            last_charting_tab = 7
            charting_tab_mode = 7
            work_zone = chord_charting_area_letters
            r.ImGui_EndTabItem(ctx)
        end
        ...
        reaper.ImGui_Dummy(ctx, 0, 3) -- Change '8' to make the gap bigger or smaller

        if charting_tab_mode == 1 or charting_tab_mode == 7 then
                    local text_area_width = 685
                    local height_with_headers = 513
                    local height_no_headers = 595
                    ...
                    if show_headers == true then
                        ...
                        local changed_h, new_h =
                            r.ImGui_InputTextMultiline(ctx, "##header_area", header_area, 541, 102, reaper.ImGui_InputTextFlags_AllowTabInput())
                        if changed_h then
                            header_area = new_h
                        end
        
                        if charting_tab_mode == 1 then
                            local changed_c, new_c = r.ImGui_InputTextMultiline(ctx, "##chord_charting_area", chord_charting_area, text_area_width, height_with_headers, reaper.ImGui_InputTextFlags_AllowTabInput())
                            if changed_c then chord_charting_area = new_c end
                        else
                            local changed_c, new_c = r.ImGui_InputTextMultiline(ctx, "##chord_charting_area_letters", chord_charting_area_letters, text_area_width, height_with_headers, reaper.ImGui_InputTextFlags_AllowTabInput())
                            if changed_c then chord_charting_area_letters = new_c end
                        end
                    else
                        ...
                        if charting_tab_mode == 1 then
                            local changed_c, new_c = r.ImGui_InputTextMultiline(ctx, "##chord_charting_area", chord_charting_area, text_area_width, height_no_headers, reaper.ImGui_InputTextFlags_AllowTabInput())
                            if changed_c then chord_charting_area = new_c end
                        else
                            local changed_c, new_c = r.ImGui_InputTextMultiline(ctx, "##chord_charting_area_letters", chord_charting_area_letters, text_area_width, height_no_headers, reaper.ImGui_InputTextFlags_AllowTabInput())
                            if changed_c then chord_charting_area_letters = new_c end
                        end
                    end
        ...
        reaper.ImGui_EndGroup(ctx) -- THIS CLOSES THE LEFT PANEL!
        r.ImGui_SameLine(ctx)

        reaper.ImGui_BeginGroup(ctx) -- THIS STARTS THE RIGHT PANEL!
...
{O}
        ...
        reaper.ImGui_Dummy(ctx, 0, 2) -- Adjust this number to push the Right tabs down!
     ...
     if r.ImGui_BeginTabItem(ctx, "Options") then
      ...
      --  feedback_zone = help.Chordsheet_output
        ...
        if feedback_tab_mode == 0 then
            ...
            reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), verylightgray)
    ...
    reaper.ImGui_EndGroup(ctx)
        ...
        if feedback_tab_mode == 4 and import_tab_mode == 1 then
            ...
            reaper.ImGui_Dummy(ctx, 0, 0)
            local midgray = reaper.ImGui_ColorConvertDouble4ToU32(1, .7, .5, 1)
            reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), midgray)
            rv, letter_to_num_key = r.ImGui_InputTextMultiline(ctx, "Key", letter_to_num_key, 35, 20, nil)
            reaper.ImGui_PopStyleColor(ctx)
            r.ImGui_SameLine(ctx)
            reaper.ImGui_Dummy(ctx, 44, 10)
            r.ImGui_SameLine(ctx)
        ...
        if feedback_tab_mode == 4 and import_tab_mode == 3 then
            ...
            local midorange = reaper.ImGui_ColorConvertDouble4ToU32(1, .7, .5, 1)
            reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), midorange)
            rv, import_key =
                r.ImGui_InputTextMultiline(
                ctx,
                "Import Key",
            ...
            reaper.ImGui_PopStyleColor(ctx)
            r.ImGui_SameLine(ctx)
            reaper.ImGui_Dummy(ctx, 32, 1)
            r.ImGui_SameLine(ctx)

            r.ImGui_SameLine(ctx)
            if r.ImGui_Button(ctx, "Convert clipboard: OneMotion >> Numbers2Notes format", nil, nil) then
                import_onemotion()
            end
            reaper.ImGui_Dummy(ctx, 0, 0)
        ...
        reaper.ImGui_EndGroup(ctx) -- THIS CLOSES THE RIGHT PANEL

        -- ==============================================================
        -- FULL WIDTH ACTION BUTTONS & MINI TRANSPORT
        -- ==============================================================
        ...
        local black_color = reaper.ImGui_ColorConvertDouble4ToU32(0, 0, 0, 1)
        local white_color = reaper.ImGui_ColorConvertDouble4ToU32(1, 1, 1, 1)
        local floating_label_y_offset = -12
        local btn_height = 22
        local mini_btn_h = 14
        local nav_btn_w = 26

        -- Helper function to draw F-Key text without triggering layout bugs
        local function DrawFloatingLabel(text, x, y, width)
            reaper.ImGui_PushFont(ctx, fontsmall)
        ...
        local btn1_w = 120
        DrawFloatingLabel("F7", cx, cy, btn1_w)
        if r.ImGui_Button(ctx, "Setup & Render", btn1_w, btn_height) then
            Action_SetupAndRender()
        end
        r.ImGui_SameLine(ctx)

        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        local btn2_w = 60
        DrawFloatingLabel("F8", cx, cy, btn2_w)
        if r.ImGui_Button(ctx, "Update", btn2_w, btn_height) then
            Action_Update()
        end
        r.ImGui_SameLine(ctx)
        reaper.ImGui_Dummy(ctx, 90, 1)
        r.ImGui_SameLine(ctx)

        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        DrawFloatingLabel("!", cx, cy, nav_btn_w)
        reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), black_color)
        reaper.ImGui_PushStyleColor(
            ctx,
            reaper.ImGui_Col_Button(),
            reaper.ImGui_ColorConvertDouble4ToU32(230 / 255.0, 210 / 255.0, 0, 1.0)
        )
        if reaper.ImGui_Button(ctx, "!", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(40349, 0)
        end
        ...
        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        DrawFloatingLabel("F9", cx, cy, nav_btn_w)
        reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), white_color)
        reaper.ImGui_PushStyleColor(
            ctx,
            reaper.ImGui_Col_Button(),
            reaper.ImGui_ColorConvertDouble4ToU32(150 / 255.0, 10 / 255.0, 10 / 255.0, 1.0)
        )
        if reaper.ImGui_Button(ctx, "●", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(1013, 0)
        end
        ...
        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        DrawFloatingLabel("F10", cx, cy, nav_btn_w)
        reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), white_color)
        reaper.ImGui_PushStyleColor(
            ctx,
            reaper.ImGui_Col_Button(),
            reaper.ImGui_ColorConvertDouble4ToU32(10 / 255.0, 150 / 255.0, 10 / 255.0, 1.0)
        )
        if reaper.ImGui_Button(ctx, "►", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(1007, 0)
        end
        ...
        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        DrawFloatingLabel("F11", cx, cy, nav_btn_w)
        reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), white_color)
        reaper.ImGui_PushStyleColor(
            ctx,
            reaper.ImGui_Col_Button(),
            reaper.ImGui_ColorConvertDouble4ToU32(180 / 255.0, 70 / 255.0, 0, 1.0)
        )
        if reaper.ImGui_Button(ctx, "■", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(1016, 0)
            reaper.Main_OnCommand(40349, 0)
        ...
        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        DrawFloatingLabel("F12", cx, cy, nav_btn_w)
        reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), white_color)
        reaper.ImGui_PushStyleColor(
            ctx,
            reaper.ImGui_Col_Button(),
            reaper.ImGui_ColorConvertDouble4ToU32(150 / 255.0, 0, 200 / 255.0, 1.0)
        )
        if reaper.ImGui_Button(ctx, "||", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(1008, 0)
        end
        ...
        reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), white_color)
        DrawFloatingLabel("Home", cx, cy, nav_btn_w)
        reaper.ImGui_PushStyleColor(
            ctx,
            reaper.ImGui_Col_Button(),
            reaper.ImGui_ColorConvertDouble4ToU32(0.1, 0.1, 0.1, 1.0)
        )
        if reaper.ImGui_Button(ctx, "▐◄", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(40042, 0)
        end
        ...
        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        DrawFloatingLabel("Pg ↓", cx, cy, nav_btn_w)
        if reaper.ImGui_Button(ctx, "←", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(40172, 0)
        end
        r.ImGui_SameLine(ctx)
        ...
        cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
        DrawFloatingLabel("End", cx, cy, nav_btn_w)
        if reaper.ImGui_Button(ctx, "►▌", nav_btn_w, btn_height) then
            reaper.Main_OnCommand(40043, 0)
        end
        reaper.ImGui_PopStyleColor(ctx, 2)

        reaper.ImGui_EndGroup(ctx) -- Close Row 1

        reaper.ImGui_Dummy(ctx, 0, 2) -- Natural Layout Spacing
...
if reaper.ImGui_BeginChild(ctx, "TransportSections", child_w, 42, false, reaper.ImGui_WindowFlags_HorizontalScrollbar()) then
                ...
                for i, sec in ipairs(sections) do
                    ...
                    if sec.is_missing then
                        reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_Alpha(), 0.3)
                        reaper.ImGui_BeginDisabled(ctx)
                    end
                    
                    reaper.ImGui_SetCursorPosY(ctx, 2)
                    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Button(), 0xAAAAAAFF)
                    ...
                    if reaper.ImGui_Button(ctx, "-2##"..i, 16, mini_btn_h) then Jump_To_Region_Index(sec.region_idx, sec.raw_text, -2) end
                    reaper.ImGui_SameLine(ctx, nil, 2)
                    reaper.ImGui_SetCursorPosY(ctx, 2)
                    if reaper.ImGui_Button(ctx, "-1##"..i, 16, mini_btn_h) then Jump_To_Region_Index(sec.region_idx, sec.raw_text, -1) end
                    
                    reaper.ImGui_PopFont(ctx)
                    reaper.ImGui_PopStyleVar(ctx, 1)
                    reaper.ImGui_PopStyleColor(ctx, 2)
                    
                    reaper.ImGui_SetCursorPosY(ctx, 18)
                    
                    local luminance = (sec.r * 0.299) + (sec.g * 0.587) + (sec.b * 0.114)
                    local text_col = (luminance < 0.45) and white_color or black_color
                    
                    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), text_col)
                    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Button(), reaper.ImGui_ColorConvertDouble4ToU32(sec.r, sec.g, sec.b, 1.0))
                    
                    local tw, th = reaper.ImGui_CalcTextSize(ctx, sec.raw_text)
                    local btn_w = math.max(34, tw + 16)
                    
                    local cx, cy = reaper.ImGui_GetCursorScreenPos(ctx)
                    
                    if reaper.ImGui_Button(ctx, sec.raw_text.." ##m"..i, btn_w, btn_height) then
                        Jump_To_Region_Index(sec.region_idx, sec.raw_text, 0)
                    end
                    ...
                    if sec.is_missing then
                        ...
                        -- Draw the big Red X and "Missing" overlay
                        local red = reaper.ImGui_ColorConvertDouble4ToU32(1.0, 0.0, 0.0, 1.0)
                        reaper.ImGui_DrawList_AddLine(draw_list, cx, cy, cx + btn_w, cy + btn_height, red, 2.0)
                        reaper.ImGui_DrawList_AddLine(draw_list, cx + btn_w, cy, cx, cy + btn_height, red, 2.0)
                        
                        reaper.ImGui_PushFont(ctx, fontsmall)
                        local msg = "Missing"
                        local msg_w = reaper.ImGui_CalcTextSize(ctx, msg)
                        reaper.ImGui_DrawList_AddTextEx(draw_list, fontsmall, font_small_size, cx + (btn_w - msg_w)/2, cy + btn_height + 2, red, msg)
                        reaper.ImGui_PopFont(ctx)
                    end
        ...
        -- Add this line to draw the Setup Modal if it has been triggered!
        Draw_Track_Builder_Modal()

        r.ImGui_End(ctx) -- Close the Main Window
    end -- THIS ENDS 'if visible then'
...
function Track_Needs_Cue_Child(tr)
    ...
    for _, fx_info in ipairs(tr[5]) do
        ...
        if fx_name and (fx_name:find("N2N Drum Arranger") or fx_name:find("N2N Arp")) then
            return true
        end
    end
...
function Organize_Tracks_And_Routing(dynamic_table)
    ...
    for i, tr in ipairs(dynamic_table) do
        ...
        local is_instrument =
            (tr.type_id == 16 and tr.mode_id > 0) or (tr.type_id == 17 and tr.mode_id > 0) or
...
function place_special()
    ...
    for i, v in ipairs(G_region_table) do
        ...
        elseif
            region_type_upper:match("^P%d*$") or region_type_upper:match("^PC%d*$") or
                region_type_upper:match("^PRE[- ]?")
         then
...
function process_pushes()
    ...
    for i = last_element, 1, -1 do
        ...
        elseif string.sub(chord_table[i][4], 1, 3) == "2t<" or string.sub(chord_table[i][4], 1, 3) == "2T<" then
            --r
eaper.ShowConsoleMsg("Two Triplet Push - " .. chord_table[i][4] .. "\n")
...
function numbers_to_letters(keysig, numbers_text)
    reaper.PreventUIRefresh(1)
    local result = numbers_text
    local key_shift = musictheory.key_table[keysig]
    if not key_shift then 
        reaper.PreventUIRefresh(-1)
        return result 
    end
    
    local is_flat = musictheory.is_it_flat_table[keysig]
    
    local final_text = string.gsub(result, "([^\r\n]*)", function(line)
        if line:match("^%s*{") or line:match("^%s*=") or line:match("^%s*$") then
            return line
        else
            return string.gsub(line, "([b#]?)([1-7])([%w_addsusmajdim]*)", function(pref, root, suff)
                local root_val = musictheory.root_table[pref..root]
                if not root_val then return pref..root..suff end
                
                local total_shift = (root_val + key_shift) % 12
                local letter = ""
                if is_flat then
                    letter = musictheory.flats_table[total_shift]
                else
                    letter = musictheory.sharps_table[total_shift]
                end
                return letter .. suff
            end)
        end
...
function letters_to_numbers(keysig, letters)
    reaper.PreventUIRefresh(1) 
    if musictheory.is_it_flat_table[keysig] ~= nil then 
        local key_number = musictheory.key_table[keysig] 
        
        local final_text = string.gsub(letters, "([^\r\n]*)", function(line)
            -- CRITICAL FIX: Skip section headers and commands completely!
            if line:match("^%s*{") or line:match("^%s*=") or line:match("^%s*$") then
                return line
            end
            
            local numbers_result = line
            for k, v in pairs(musictheory.conflict_table) do 
                numbers_result = string.gsub(numbers_result, k, v)
            end
            for i, v in pairs(musictheory.full_letter_list_set) do
                local currentletter = v[1] 
                local shifted_key_number = v[2] - key_number 
                if shifted_key_number < 0 then shifted_key_number = shifted_key_number + 12 end 
                local replacing_number = musictheory.reverse_root_table[shifted_key_number] 
                numbers_result = string.gsub(numbers_result, currentletter, replacing_number) 
            end
            ...
            local slenght = 0
            while string.len(numbers_result) ~= slenght do
...
function Start_Audition_Mute_Timer()
    Kill_All_Audition_Notes() -- Instantly send note-offs to start release phase
    audition_mute_time = reaper.time_precise() + (audition_mute_delay_ms / 1000.0)
end

function Cancel_Audition_Mute_Timer()
    audition_mute_time = nil
    if audition_track and reaper.ValidatePtr(audition_track, "MediaTrack*") then
        reaper.SetMediaTrackInfo_Value(audition_track, "B_MUTE", 0) -- Unmute the track instantly
    end
end
...
function play_button_midi(v_in, play_root_in, do_glow)
    ...
    if r.ImGui_IsItemActivated(ctx) then
        ...
        if audition_track and reaper.ValidatePtr(audition_track, "MediaTrack*") then
            reaper.SetMediaTrackInfo_Value(audition_track, "B_MUTE", 0)
            audition_mute_time = nil -- Cancel any pending mute timer
        end
...
local old_len = 0
    ...
    -- CONVERT RETURNS TO SPACES
    old_len = 0
    while old_len ~= string.len(unfolded_ccc_data) do
        old_len = string.len(unfolded_ccc_data)
        unfolded_ccc_data = string.gsub(unfolded_ccc_data, "\n", " ")
    end
    ...
    -- SEPARATE BRACED MEASURES BY A SPACE
    old_len = 0
    while old_len ~= string.len(unfolded_ccc_data) do
        old_len = string.len(unfolded_ccc_data)
        unfolded_ccc_data = string.gsub(unfolded_ccc_data, "%]%[", "] [")
    ...
    -- TRIM OUT SPACES FROM INSIDE IN BRACES
    old_len = 0
    while old_len ~= string.len(unfolded_ccc_data) do
        old_len = string.len(unfolded_ccc_data)
        unfolded_ccc_data = string.gsub(unfolded_ccc_data, "%[ ", "[")
    ...
    -- TRIM OUT SPACES FROM INSIDE OUT BRACES
    old_len = 0
    while old_len ~= string.len(unfolded_ccc_data) do
        old_len = string.len(unfolded_ccc_data)
        unfolded_ccc_data = string.gsub(unfolded_ccc_data, " %]", "]")
    ...
    -- CONVERT ALL REST TO Chordsheet REST 1 (rr)
    old_len = 0
    while old_len ~= string.len(unfolded_ccc_data) do
        old_len = string.len(unfolded_ccc_data)
    ...
    -- CONVERT ALL REST TO Chordsheet REST 1 (r)
    old_len = 0
    while old_len ~= string.len(unfolded_ccc_data) do
        old_len = string.len(unfolded_ccc_data)
